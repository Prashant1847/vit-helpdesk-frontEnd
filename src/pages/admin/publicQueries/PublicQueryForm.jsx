...
const PublicQueryForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(import.meta.env.VITE_BASE_URL + `/api/admin/queries/${id}/public`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.message || 'Failed to make query public');
      }

      navigate('/admin');
      toast.success('Query made public successfully');
    } catch (error) {
      toast.error(error.message || 'Failed to make query public');
    }
  };
