...
const PublicQueryManagement = () => {
  ...
  const fetchPublicQueries = async () => {
    try {
      ...
      const response = await fetch(import.meta.env.VITE_BASE_URL + `/api/admin/public/queries?${queryParams.toString()}`, {
        credentials: 'include'
      });
      
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.message || `Failed to fetch public queries`);
      }

      const data = await response.json();
      setQueries(data);
    } catch (error) {
      setError(error.message || 'Failed to fetch public queries');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemove = async (queryId) => {
    try {
      const response = await fetch(import.meta.env.VITE_BASE_URL + `/api/admin/queries/${queryId}/private`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        toast.success('Query removed from public view');
        setTimeout(() => setSuccessMessage(''), 3000);
      } else {
        const data = await response.json();
        throw new Error(data.message || 'Failed to remove query');
      }
    } catch (error) {
